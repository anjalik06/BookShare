...
const Profile: React.FC = () => {
  ...
  const fetchProfileData = async () => {
    ...
    try {
      const [resAll, resOnLoan, resBorrowed, resRequests] =
        await Promise.all([
          api.post<Book[]>("/api/books/user", { userId: user._id }),
          api.get<BookRequest[]>(`/api/books/onloan/${user._id}`),
          api.get<BookRequest[]>(`/api/books/borrowed/${user._id}`),
          api.get<BookRequest[]>(`/api/books/requests/user/${user._id}`),
        ]);
  ...
  const handleApprove = async (bookId: string, requesterId: string) => {
    try {
      await api.post(`/api/books/${bookId}/request/${requesterId}/approve`);
      setRequests(prev =>
        prev.filter(
          r => r.bookId !== bookId || r.requester._id !== requesterId
        )
      );
    } catch {
  ...
  const handleReject = async (bookId: string, requesterId: string) => {
    try {
      await api.post(`/api/books/${bookId}/request/${requesterId}/reject`);
      setRequests(prev =>
        prev.filter(
          r => r.bookId !== bookId || r.requester._id !== requesterId
        )
